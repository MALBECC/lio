...
c-----------------------------------------------------------------
      subroutine int22(NORM,natom,r,Nucd,M,Md,ncontd,nshelld,cd,ad,
     > RMM,XX)
c
...
c
c
       call g2g_timer_start('dppco+dppdi+loop')
c LINPACK OPTION
#ifdef pack
      call g2g_timer_start('parte')
c
      kk=0
      do 313 j=1,Md
       do 313 i=1,j
       ...
       RMM(M9+kk-1)=RMM(kx)
 313  continue
      call g2g_timer_stop('parte')
c
      call g2g_timer_start('dppco')
      call dppco(RMM(M9),Md,rcond,aux,info)
      call g2g_timer_stop('dppco')
      call g2g_timer_start('dppdi')
      call dpptri('U', Md, RMM(M9), info)
c      call dppdi(RMM(M9),Md,det,1)
      call g2g_timer_stop('dppdi')
c
      call g2g_timer_start('resto')
      kk=0
      do 314 j=1,Md
      do 314 i=1,j
      kk=kk+1
...
c
      call g2g_timer_stop('resto')
#endif
c
      call g2g_timer_stop('dppco+dppdi+loop')
      endif
