...
c Dario Estrin
c-------------------------------------------------------------------
      subroutine int1G(NORM,natom,r,Nuc,Iz,M,Md,ncont,nshell,
     >            c,a,RMM,En,ff,FXH2,FYH2,FZH2)
...
c
      dimension Q(3),d(ntq,ntq),s0s(ntq),s1s(ntq),
     >            s2s(ntq),s3s(ntq),s4s(ntq)
      dimension s5s(ntq),s6s(ntq)
      dimension x0x(ntq,3),x1x(ntq,3),x2x(ntq,3)
      dimension dn(3),dn1(3),dn2(3)
      dimension dn3(3),dn4(3),dn5(3),dn6(3),dn7(3)
      dimension dn8(3),dn9(3),dn10(3)
...
c-----Control de fzas en int1G 
      ...
      fyy=fyy+ff(i,2)
      fzz=fzz+ff(i,3)
      write(*,*)'INT1G:fx fy fz',i,ff(i,1),ff(i,2),ff(i,3)
      enddo
      
      write(*,*)'INT1G FTOT',dsqrt(fxx**2+fyy**2+fzz**2)

      if(dsqrt(fxx**2+fyy**2+fzz**2).gt.1.D-07)then
      write(*,*)'FZA TOTAL NE ZERO EN INT1G'  
      endif
990   CONTINUE
c----------------------------------------------------------
