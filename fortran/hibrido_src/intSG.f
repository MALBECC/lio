...
c Dario Estrin
c-------------------------------------------------------------------
      subroutine intSG(NORM,natom,nsol,natsol,r,Nuc,M,Md,ncont,nshell,
...
c
     ...
     >          rpi=1.77245385090551588D0)
      ...
      dimension r(nt,3),nshell(0:3),Ll(3)
      dimension ff(nt,3)
      dimension RMM(*)
c
...
c     write(*,*) 'con =',con
c      do i=1,natom
c      write(*,*) 'en intSG',i,ff(i,1),ff(i,2),ff(i,3)
c      enddo
c
...
c
      ...
      do i=1,natom
       ff(i,1)=ff(i,1)-Pm(i)*fx/Pp
       ff(i,2)=ff(i,2)-Pm(i)*fy/Pp
       ff(i,3)=ff(i,3)-Pm(i)*fz/Pp
      enddo
c
c     pause
c
      ...
      IF(1.NE.2)GOTO 990
c-----Control de fzas en intSG
      zero=0.D0
      fxx=zero
      fyy=zero
      fzz=zero
      
      do i=1,nsol*natsol+natom
      fxx=fxx+ff(i,1)
      fyy=fyy+ff(i,2)
      fzz=fzz+ff(i,3)
      write(*,*)'INTSG fx fy fz',i,ff(i,1),ff(i,2),ff(i,3)
      enddo

      write(*,*)'INTSG FTOT',dsqrt(fxx**2+fyy**2+fzz**2)

      if(dsqrt(fxx**2+fyy**2+fzz**2).gt.1.D-07)then
      write(*,*)'FZA TOTAL NE ZERO EN INTSG' 
