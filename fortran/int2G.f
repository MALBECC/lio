...
c-----------------------------------------------------------------
      subroutine int2G(NORM,natom,r,Nucd,M,Md,ncontd,nshelld,cd,ad,
     >           cf,f)
c
...
c
       ...
       cc=-0.5D0*fac*cf(ii)*cf(jj)*ccoef/(f1*f2)
       cc1=cc*2.D0
       cci=cc1*ad(i,ni)
       ccj=cc1*ad(j,nj)
c
...
c     do 178 j=1,i
c      k=i+((Md2-j)*(j-1))/2
c 178  cont=cont+cf(i)*cf(j)*RMM(M7+k-1)
c     do 179 j=i+1,nsd
c      k=j+((Md2-i)*(i-1))/2
c 179  cont=cont+cf(i)*cf(j)*RMM(M7+k-1)
c 177  continue
c
c     do 277 i=nsd+1,nsd+npd
c     do 278 j=nsd+1,i
c      k=i+((Md2-j)*(j-1))/2
c 278  cont=cont+cf(i)*cf(j)*RMM(M7+k-1)
c     do 279 j=i+1,nsd+npd
c      k=j+((Md2-i)*(i-1))/2
c 279  cont=cont+cf(i)*cf(j)*RMM(M7+k-1)
c 277  continue
c
c     do 377 i=nsd+npd+1,Md
c     do 378 j=nsd+npd+1,i
c      k=i+((Md2-j)*(j-1))/2
c 378  cont=cont+cf(i)*cf(j)*RMM(M7+k-1)
c     do 379 j=i+1,Md
c      k=j+((Md2-i)*(i-1))/2
c 379  cont=cont+cf(i)*cf(j)*RMM(M7+k-1)
c 377  continue
c
c      do 477 i=nsd+1,nsd+npd
c      do 477 j=1,nsd
c      k=i+((Md2-j)*(j-1))/2
c477    cont=cont+2.D0*cf(i)*cf(j)*RMM(M7+k-1)
c
c      do 577 i=nsd+npd+1,Md
c      do 577 j=1,nsd
c      k=i+((Md2-j)*(j-1))/2
c577    cont=cont+2.D0*cf(i)*cf(j)*RMM(M7+k-1)
c
c      do 677 i=nsd+npd+1,Md
c      do 677 j=nsd+1,nsd+npd
c      k=i+((Md2-j)*(j-1))/2
c677    cont=cont+2.D0*cf(i)*cf(j)*RMM(M7+k-1)
c
c       cont=-0.5D0*cont
c     write(*,*) 'cont =',cont
c     do i=1,natom
