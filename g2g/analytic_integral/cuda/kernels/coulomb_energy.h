...
__global__ void gpu_coulomb_fock(
        ...
        for (int j = 0;
             ...
             j += inner_step[func_type]) {
          {
            ...
            scalar_type inv_two_eta, rho_eta;
            {
              ...
              // if (i+j < term_end[0]) {
              //  prefactor_dens = (double)(fit_dens_sh[j]*ac_val_dens_sh[j].y)
...
//
// Do the core part of the forces calculation - the evaluation of the
// Obara-Saika recursion equations
// This is where the different term types differ the most, so these are moved
// into separate files in the qmmm_terms directory
// Current version: p-s through d-d are manually unrolled, and d-d is split up
// over six threads per primitive pair
//
