...
//////////////////////////////
    ...
    // TODO: each thread calculates its own zeta, overlap, etc here; should these be precalculated and saved (for use here and in Coulomb calculation)?
    {
      ...
      uint nuc1, nuc2;
      {
        ...
        //
        G2G::vec_type<scalar_type,2> ac1 = ac_values[f1 + cont1 * COALESCED_DIMENSION(gpu_m)];
        G2G::vec_type<scalar_type,2> ac2 = ac_values[f2 + cont2 * COALESCED_DIMENSION(gpu_m)];
        ai = ac1.x;
        aj = ac2.x;
        cc = ac1.y * ac2.y;
