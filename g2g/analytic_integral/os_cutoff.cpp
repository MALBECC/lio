        ...
        for (ni = 0; ni < G2G::fortran_vars.contractions(i); ni++) {
          ...
          for (nj = 0; nj < G2G::fortran_vars.contractions(j); nj++) {
            ...
            if (!this->apply_cutoff || dsq*ksi < integral_vars.rmax) {
              ...
              this_func_code     += i  * MAX_CONTRACTIONS * MAX_CONTRACTIONS * G2G::fortran_vars.m;  // Finally, function # i in the highest part

              this->func_code.push_back(this_func_code); // Which primitives the thread represents
              this->local_dens.push_back(local_dens_ind); // Which part of the (reduced) density matrix the thread needs
        ...
        if (use_funcs) {
          for (uint i_orbital = 0; i_orbital < i_orbitals; i_orbital++) {
            ...
            for (uint j_orbital = 0; j_orbital < j_orbital_finish; j_orbital++) {
              ...
              uint dens_ind = (i+i_orbital) + (2*G2G::fortran_vars.m-((j+j_orbital)+1))*(j+j_orbital)/2;
              this->dens_values.push_back(G2G::fortran_vars.rmm_input_ndens1.data[dens_ind]);
