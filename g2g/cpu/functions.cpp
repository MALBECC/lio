    ...
    for (list<Point>::const_iterator point_it = group.points.begin(); point_it != group.points.end(); ++point_it, ++point) {
      ...
      for (uint i = 0, ii = 0; i < group.functions.size(); i++) {
        ...
        if (i < group.s_functions) {
          ...
          if (forces || gga) {
            group.gradient_values.get(ii, point) = v * (-2 * tg);
          }
          if (gga) {
            group.hessian_values.get(2 * ii + 0, point) = (v * v) * 4 * th - 2 * tg; // Fxx, Fxy, Fxz
            group.hessian_values.get(2 * ii + 1, point) = vxxy * vyzz * 4 * th; // Fxy, Fxz, Fyz
          }
          ...
          if (forces || gga) {
            group.gradient_values.get(ii + 0, point) = make_float3(t, 0, 0) - v * 2 * tg * v.x;
            group.gradient_values.get(ii + 1, point) = make_float3(0, t, 0) - v * 2 * tg * v.y;
            group.gradient_values.get(ii + 2, point) = make_float3(0, 0, t) - v * 2 * tg * v.z;
          }
          if (gga) {
            group.hessian_values.get(2 * (ii + 0) + 0, point) = 4 * th * v.x * (v * v) - tg * v.x * make_float3(6, 2, 2);
          ...
          if (forces || gga) {
            group.gradient_values.get(ii + 0, point) = (make_float3(2 * v.x, 0      , 0      ) * t - 2 * tg * v * v.x * v.x) * fortran_vars.normalization_factor;
            group.gradient_values.get(ii + 1, point) =  make_float3(v.y    , v.x    , 0      ) * t - 2 * tg * v * v.y * v.x;
            group.gradient_values.get(ii + 2, point) = (make_float3(0      , 2 * v.y, 0      ) * t - 2 * tg * v * v.y * v.y) * fortran_vars.normalization_factor;
            group.gradient_values.get(ii + 3, point) =  make_float3(v.z    , 0      , v.x    ) * t - 2 * tg * v * v.z * v.x;
            group.gradient_values.get(ii + 4, point) =  make_float3(0      , v.z    , v.y    ) * t - 2 * tg * v * v.z * v.y;
            group.gradient_values.get(ii + 5, point) = (make_float3(0      , 0      , 2 * v.z) * t - 2 * tg * v * v.z * v.z) * fortran_vars.normalization_factor;
          }
          ii += 6;
        }
      }
