     ...
     HostMatrix<double>& rmm_output_local_b)
{
  ...
  if(compute_forces) {
    ...
    forces_mat_a.resize(this->points.size(), 
                        vector<vec_type3>(this->total_nucleii(), vec_type3(0.f,0.f,0.f)));
    forces_mat_b.resize(this->points.size(), 
                        vector<vec_type3>(this->total_nucleii(), vec_type3(0.f,0.f,0.f)));
  }
  ...
  if (lda) {
    ...
    for(int point = 0; point < npoints; point++) {
      ...
      // Partial density and space derivatives values in point (accumulates functions)
      scalar_type pd_a, pd_b;
      scalar_type tdx_a  , tdy_a  , tdz_a  , tdx_b  , tdy_b  , tdz_b;   // dx, dy, dz
      scalar_type tdd1x_a, tdd1y_a, tdd1z_a, tdd1x_b, tdd1y_b, tdd1z_b; // dxx, dyy, dzz
      scalar_type tdd2x_a, tdd2y_a, tdd2z_a, tdd2x_b, tdd2y_b, tdd2z_b; // dxy, dxz, dyz

      pd_a = tdx_a = tdy_a = tdz_a = tdd1x_a = tdd1y_a = tdd1z_a = tdd2x_a = tdd2y_a = tdd2z_a = 0;
      pd_b = tdx_b = tdy_b = tdz_b = tdd1x_b = tdd1y_b = tdd1z_b = tdd2x_b = tdd2y_b = tdd2z_b = 0;
      ...
      for (int i = 0; i < group_m; i++) {
        ...
        w_a = w3x_a = w3y_a = w3z_a = ww1x_a = ww1y_a = ww1z_a = ww2x_a = ww2y_a = ww2z_a = 0;
        w_b = w3x_b = w3y_b = w3z_b = ww1x_b = ww1y_b = ww1z_b = ww2x_b = ww2y_b = ww2z_b = 0;
        ...
        for(int j = 0; j <= i; j++) {
          ...
          w_a += fv[j] * rmm_aj;
          w3x_a  += gxv[j]  * rmm_aj; w3y_a  += gyv[j]  * rmm_aj; w3z_a  += gzv[j]  * rmm_aj;
          ww1x_a += hpxv[j] * rmm_aj; ww1y_a += hpyv[j] * rmm_aj; ww1z_a += hpzv[j] * rmm_aj;
          ww2x_a += hixv[j] * rmm_aj; ww2y_a += hiyv[j] * rmm_aj; ww2z_a += hizv[j] * rmm_aj;

          w_b += fv[j] * rmm_bj;
          w3x_b  += gxv[j]  * rmm_bj; w3y_b  += gyv[j]  * rmm_bj; w3z_b  += gzv[j]  * rmm_bj;
          ww1x_b += hpxv[j] * rmm_bj; ww1y_b += hpyv[j] * rmm_bj; ww1z_b += hpzv[j] * rmm_bj;
          ww2x_b += hixv[j] * rmm_bj; ww2y_b += hiyv[j] * rmm_bj; ww2z_b += hizv[j] * rmm_bj;
        }
      ...
      const vec_type3 dd1_a(tdd1x_a, tdd1y_a, tdd1z_a), dd1_b(tdd1x_b, tdd1y_b, tdd1z_b);
      const vec_type3 dd2_a(tdd2x_a, tdd2y_a, tdd2z_a), dd2_b(tdd2x_b, tdd2y_b, tdd2z_b);
     
      calc_ggaOS<scalar_type, 3>(pd_a, pd_b, dxyz_a, dxyz_b, dd1_a, dd1_b, dd2_a, dd2_b,
                                 exc_corr, exc, corr, corr1, corr2, y2a, y2b, 9);
  ...
  if (compute_forces) {
    ...
    ddx_a.resize(this->total_nucleii(), 1); ddx_b.resize(this->total_nucleii(), 1);
    ddy_a.resize(this->total_nucleii(), 1); ddy_b.resize(this->total_nucleii(), 1);
    ddz_a.resize(this->total_nucleii(), 1); ddz_b.resize(this->total_nucleii(), 1);
    ...
    for(int point = 0; point < this->points.size(); point++) {
      ...
      for (int i = 0, ii = 0; i < this->total_functions_simple(); i++) {
        ...
        for (uint k = 0; k < inc_i; k++, ii++) {
          ...
          tddx_a -= w_a*gX(ii, point); tddx_b -= w_b*gX(ii, point);
          tddy_a -= w_a*gY(ii, point); tddy_b -= w_b*gY(ii, point);
          tddz_a -= w_a*gZ(ii, point); tddz_b -= w_b*gZ(ii, point);
        }
        ddx_a(nuc) += tddx_a; ddy_a(nuc) += tddy_a; ddz_a(nuc) += tddz_a;
        ddx_b(nuc) += tddx_b; ddy_b(nuc) += tddy_b; ddz_b(nuc) += tddz_b;
      }

      scalar_type factor_a = factors_rmm_a(point);
      scalar_type factor_b = factors_rmm_b(point);
      for (int i = 0; i < this->total_nucleii(); i++) {
        forces_mat_a[point][i] = vec_type3(ddx_a(i), ddy_a(i), ddz_a(i)) * factor_a;
        forces_mat_b[point][i] = vec_type3(ddx_b(i), ddy_b(i), ddz_b(i)) * factor_b;
      }
    }
