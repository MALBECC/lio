...
#define BLOCK_DIM 16
  ...
  cudaArray* cuArray;
  cudaMallocArray(&cuArray, &rmm_input_gpu_tex.channelDesc, rmm_input_cpu.width,
...
#undef libxc_init_param
  ...
  if (((my_cdft_vars.do_chrg || my_cdft_vars.do_spin) && compute_rmm) ||
    ...
    HostMatrix<scalar_type> becke_w_cpu(fortran_vars.atoms *
...
#define accumulate_parameters                                           \
  energy_gpu.data, factors_gpu.data, point_weights_gpu.data,            \
      ...
      dxyz_gpu.data, dd1_gpu.data, dd2_gpu.data, fortran_vars.fexc
      ...
      if (lda) {
        ...
        if (fortran_vars.use_libxc) {
          ...
          libxc_exchange_correlation_gpu<scalar_type, true, true, false>(
              &libxcProxy_cuda, energy_gpu.data, factors_gpu.data,
      ...
      if (my_cdft_vars.do_chrg) {
        ...
        gpu_cdft_factors<scalar_type>
            <<<threadGrid_accumulate, threadBlock_accumulate>>>(
                ...
                point_weights_gpu.data, becke_w_gpu.data,
                this->number_of_points, fortran_vars.atoms,
                my_cdft_vars.regions, my_cdft_vars.max_nat);
      }
    } else {
      if (lda) {
    ...
    if (fortran_vars.becke) {
      CudaMatrix<scalar_type> becke_dens_gpu(fortran_vars.atoms *
                                             this->number_of_points);
      becke_dens_gpu.zero();
      gpu_compute_becke_cs<scalar_type>
          <<<threadGrid_accumulate, threadBlock_accumulate>>>(
              becke_dens_gpu.data, partial_densities_gpu.data,
      ...
      for (unsigned int jpoint = 0; jpoint < this->number_of_points; jpoint++) {
        for (unsigned int iatom = 0; iatom < fortran_vars.atoms; iatom++) {
          becke_dens(iatom) +=
              (double)becke_dens_cpu(jpoint * fortran_vars.atoms + iatom);
        }
      }
    }
  } else {
...
#define accumulate_parameters                                                \
  NULL, factors_gpu.data, point_weights_gpu.data, this->number_of_points,    \
      ...
      dd2_gpu.data, fortran_vars.fexc
    if (lda) {
      ...
      if (fortran_vars.use_libxc) {
        ...
        libxc_exchange_correlation_gpu<scalar_type, false, true, false>(
            &libxcProxy_cuda, NULL, factors_gpu.data, this->number_of_points,
...
#undef accumulate_parameters
  ...
  if (compute_forces) {
    ...
    CudaMatrix<vec_type4> dd_gpu(COALESCED_DIMENSION(this->number_of_points),
                                 this->total_nucleii());
    dd_gpu.zero();
    ...
    gpu_compute_density_derivs<<<threadGrid, threadBlock>>>(
        function_values.data, gradient_values.data, nuc_gpu.data, dd_gpu.data,
  ...
  if (compute_rmm) {
    ...
    CudaMatrix<scalar_type> rmm_output_gpu(COALESCED_DIMENSION(group_m),
                                           group_m);
    rmm_output_gpu.zero();
...
#define BLOCK_DIM 16
  ...
  HostMatrix<scalar_type> rmm_input_a_cpu(COALESCED_DIMENSION(group_m),
                                          group_m + DENSITY_BLOCK_SIZE);
  HostMatrix<scalar_type> rmm_input_b_cpu(COALESCED_DIMENSION(group_m),
                                          group_m + DENSITY_BLOCK_SIZE);
  ...
  */
  ...
  cudaArray* cuArray1;
  cudaArray* cuArray2;
  cudaMallocArray(&cuArray1, &rmm_input_gpu_tex.channelDesc,
                  rmm_input_a_cpu.width, rmm_input_a_cpu.height);
  cudaMallocArray(&cuArray2, &rmm_input_gpu_tex2.channelDesc,
                  rmm_input_b_cpu.width, rmm_input_b_cpu.height);
  cudaMemcpyToArray(
  ...
  if (compute_energy) {
    ...
    if (compute_forces || compute_rmm) {
      gpu_compute_density_opened<scalar_type, true, true, false>
          <<<threadGrid, threadBlock>>>(
              ...
              function_values_transposed.data, gradient_values_transposed.data,
              hessian_values_transposed.data, group_m,
      ...
      gpu_accumulate_point_open<scalar_type, true, true, false>
          <<<threadGrid_accumulate, threadBlock_accumulate>>>(
              energy_gpu.data, factors_a_gpu.data, factors_b_gpu.data,
              point_weights_gpu.data, this->number_of_points, block_height,
              partial_densities_a_gpu.data, dxyz_a_gpu.data, dd1_a_gpu.data,
              dd2_a_gpu.data, partial_densities_b_gpu.data, dxyz_b_gpu.data,
              dd1_b_gpu.data, dd2_b_gpu.data, fortran_vars.fexc);
    ...
    if (fortran_vars.becke) {
      CudaMatrix<scalar_type> becke_dens_gpu(fortran_vars.atoms *
                                             this->number_of_points);
      CudaMatrix<scalar_type> becke_spin_gpu(fortran_vars.atoms *
                                             this->number_of_points);
      becke_dens_gpu.zero();
      becke_spin_gpu.zero();
      gpu_compute_becke_os<scalar_type>
          <<<threadGrid_accumulate, threadBlock_accumulate>>>(
              becke_dens_gpu.data, becke_spin_gpu.data,
      ...
      for (unsigned int jpoint = 0; jpoint < this->number_of_points; jpoint++) {
        for (unsigned int iatom = 0; iatom < fortran_vars.atoms; iatom++) {
          becke_dens(iatom) +=
              (double)becke_dens_cpu(jpoint * fortran_vars.atoms + iatom);
          becke_spin(iatom) +=
              (double)becke_spin_cpu(jpoint * fortran_vars.atoms + iatom);
        }
      }
    }
  } else {
    ...
    gpu_accumulate_point_open<scalar_type, false, true, false>
        <<<threadGrid_accumulate, threadBlock_accumulate>>>(
            NULL, factors_a_gpu.data, factors_b_gpu.data,
            point_weights_gpu.data, this->number_of_points, block_height,
            partial_densities_a_gpu.data, dxyz_a_gpu.data, dd1_a_gpu.data,
            dd2_a_gpu.data, partial_densities_b_gpu.data, dxyz_b_gpu.data,
            dd1_b_gpu.data, dd2_b_gpu.data, fortran_vars.fexc);
  ...
  if (compute_forces) {
    ...
    CudaMatrix<vec_type4> dd_gpu_a(COALESCED_DIMENSION(this->number_of_points),
                                   this->total_nucleii());
    CudaMatrix<vec_type4> dd_gpu_b(COALESCED_DIMENSION(this->number_of_points),
                                   this->total_nucleii());
    dd_gpu_a.zero();
    dd_gpu_b.zero();
    ...
    gpu_compute_density_derivs_open<<<threadGrid, threadBlock>>>(
        function_values.data, gradient_values.data, nuc_gpu.data, dd_gpu_a.data,
        dd_gpu_b.data, this->number_of_points, group_m, this->total_nucleii());
  ...
  if (compute_rmm) {
    ...
    // Only use enough blocks for lower triangle
    threadGrid = dim3(blocksPerRow * (blocksPerRow + 1) / 2);
    CudaMatrix<scalar_type> rmm_output_a_gpu(COALESCED_DIMENSION(group_m),
                                             group_m);
    CudaMatrix<scalar_type> rmm_output_b_gpu(COALESCED_DIMENSION(group_m),
                                             group_m);
...
#define BLOCK_DIM 16
    dim3 transpose_threads(BLOCK_DIM, BLOCK_DIM, 1);
    dim3 transpose_grid =
        dim3(transposed_width / BLOCK_DIM, divUp((group_m)*2, BLOCK_DIM), 1);
    hessian_values_transposed.resize(
        (group_m)*2, COALESCED_DIMENSION(this->number_of_points));
    transpose<<<transpose_grid, transpose_threads>>>(
        hessian_values_transposed.data, hessian_values.data,
        COALESCED_DIMENSION(this->number_of_points), (group_m)*2);
  }
