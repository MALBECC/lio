...
__global__ void gpu_accumulate_point_open(scalar_type* const energy, scalar_type* const energy_i, scalar_type* const energy_c,scalar_type* const energy_c1,scalar_type* const energy_c2,
                                    ...
                                    vec_type<scalar_type,WIDTH>* dd1_b, vec_type<scalar_type,WIDTH>* dd2_b)
{
  ...
  gpu_potop<scalar_type>(_partial_density_a,_partial_density_b,_dxyz_a,_dxyz_b,_dd1_a,_dd1_b,_dd2_a,_dd2_b,exc_corr,exc,corr,corr1,corr2,v_a,v_b);

  //printf("exc_corr= %.4e ", exc_corr);
  //printf("v_a= %.4e ", v_a);
  //printf("v_b= %.4e ", v_b);
...
//    printf("%.4e ",energy[point]);
  }
  if (compute_factor && valid_thread){
    factor_a[point] = point_weight * v_a;
    factor_b[point] = point_weight * v_b;
    //printf("den_a,den_b,v_a,v_b,factor_a,factor_b %13.10f %13.10f %13.10e %13.10e %13.10e %13.10e\n",_partial_density_a,_partial_density_b,v_a,v_b,factor_a[point],factor_b[point]);
    //printf("den_a,den_b,v_a,v_b, %13.10f %13.10f %13.10e %13.10e\n",_partial_density_a,_partial_density_b,v_a,v_b);
  }
}
...
template<class scalar_type, bool compute_energy, bool compute_factor, bool lda>
__global__ void gpu_accumulate_point(scalar_type* const energy,scalar_type* const energy_Ei,scalar_type* const energy_Ec,
                                     scalar_type* const energy_Ec1,scalar_type* const energy_Ec2,
  ...
  scalar_type point_weight = 0.0f;
  scalar_type y2a, exc_corr,exc,corr,corr1,corr2;
  ...
  if (valid_thread)
  {
     ...
     {
         const int this_row=j*points+point;
         //printf("partial_density= %.4e ",partial_density);
  ...
  //printf("%.4e ",_partial_density);
  gpu_pot<scalar_type, compute_energy, true, lda>(_partial_density, _dxyz, _dd1, _dd2, exc_corr,exc,corr,corr1,corr2, y2a); // TODO: segundo parametro, que tenga en cuenta RMM+energy

  if (compute_energy && valid_thread){
    energy[point]     = (_partial_density * point_weight) * exc_corr;
    energy_Ei[point]  = (_partial_density * point_weight) * exc;
    energy_Ec[point]  = (_partial_density * point_weight) * corr;
    energy_Ec1[point] = (_partial_density * point_weight) * corr1;
    energy_Ec2[point] = (_partial_density * point_weight) * corr2;
    //printf("%.4e ",energy[point]);
  }
  if (compute_factor && valid_thread)
