...
__global__ void gpu_coulomb_fit1( uint num_terms, vec_type<scalar_type,2>* ac_values, uint* func2nuc, scalar_type* dens_values, uint* func_code, uint* local_dens,
                                    double* rc_partial, uint global_stride, vec_type<scalar_type,2>* ac_values_dens, vec_type<scalar_type,3>* nuc_pos_dens,
...
__global__ void gpu_coulomb_rc_reduce( double* rc_partial,uint stride,uint width,uint num_terms )
{
  ...
  if (my_rc_ind < width) {
    ...
    //printf("G2G RC %d: %12.6e\n",my_rc_ind,my_partial_rc);
  }

}

template<class scalar_type>
__global__ void gpu_coulomb_fit2( double* rc, scalar_type* fit_dens, scalar_type* G_inv, uint* input_ind, uint m_dens )
{
  ...
  for (int i = 0; i < m_dens; i += QMMM_BLOCK_SIZE)
  {
    ...
    {
      //if (ffnum == 504) printf("G2G 505: %12.6e %12.6e\n",rc_sh[j],G_inv[(i+j)*COALESCED_DIMENSION(m_dens)+ffnum*valid_thread]);
      my_fit_dens += rc_sh[j] * (double)G_inv[(i+j)*COALESCED_DIMENSION(m_dens)+ffnum*valid_thread];
    }
    __syncthreads();
  }
  fit_dens[af_ind] = my_fit_dens*valid_thread;
  //if (valid_thread) printf("G2G AF %d: %12.6e\n",ffnum,my_fit_dens);
}
