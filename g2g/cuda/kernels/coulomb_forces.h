...
__device__ __constant__ uint TERM_TYPE_GAUSSIANS_COUL[6] = { 1, 3, 9, 6, 18, 36 }; // How many individual force terms each type (s-s,etc) is calculating
...
__global__ void gpu_coulomb_forces( uint num_terms, vec_type<scalar_type,2>* ac_values, uint* func2nuc, scalar_type* dens_values, uint* func_code, uint* local_dens,
                                    vec_type<scalar_type,3>* qm_forces, uint global_stride, vec_type<scalar_type,2>* ac_values_dens, vec_type<scalar_type,3>* nuc_pos_dens,
                                    uint* nuc_ind_dens, scalar_type* fit_dens )
{
  ...
  {
    __shared__ vec_type<scalar_type,3> nuc_pos_dens_sh[QMMM_BLOCK_SIZE];
    __shared__ uint nuc_ind_dens_sh[QMMM_BLOCK_SIZE];
    __shared__ vec_type<scalar_type,2> ac_val_dens_sh[QMMM_BLOCK_SIZE];
    ...
    // TODO: each thread calculates its own zeta, overlap, etc here; should these be precalculated and saved (for use here and in Coulomb calculation)?
    {
      scalar_type cc;
      {
        ...
        if (term_type == 2 && same_func) {
          ...
          for (uint i = 0; i < TERM_TYPE_GAUSSIANS_COUL[term_type]; i++) {
            dens[i] = dens_values[dens_ind+i];
          }
        }
    ...
    //
    for (int i = 0; i < gpu_dens_gauss; i += QMMM_BLOCK_SIZE)
    {
      if (i + tid < gpu_dens_gauss) {
      ...
      //
      for (int j = 0; j < QMMM_BLOCK_SIZE && i+j < gpu_dens_gauss; j++)
      {
        {
        ...
        if (tid == 0)               { 
          //if (valid_thread) printf("%d %.4e %.4e\n",nuc_ind_dens_sh[j],C_force[0][0],qm_forces[global_stride*nuc_ind_dens_sh[j]+blockIdx.x].x);
          qm_forces[global_stride*nuc_ind_dens_sh[j]+blockIdx.x].x += C_force[0][0]; }
        else if (tid == WARP_SIZE)  { qm_forces[global_stride*nuc_ind_dens_sh[j]+blockIdx.x].y += C_force[1][0]; }
        else if (tid == WARP_SIZE2) { qm_forces[global_stride*nuc_ind_dens_sh[j]+blockIdx.x].z += C_force[2][0]; }
        //
        // END reduction
        //
  ...
  // TODO: (same question as for the MM forces) - should we do the per-block reduction in this kernel?
  {
    ...
    for (int i = 0; i < gpu_atoms; i++)
    {
      ...
      if (nuc_flags[i] == true)
      {
        ...
        QM_force[0][tid] = valid_thread * prefactor_mo * (useA * A_force[0] + useB * B_force[0]);
        QM_force[1][tid] = valid_thread * prefactor_mo * (useA * A_force[1] + useB * B_force[1]);
        QM_force[2][tid] = valid_thread * prefactor_mo * (useA * A_force[2] + useB * B_force[2]);
        __syncthreads();
...
__global__ void zero_forces(vec_type<scalar_type,3>* forces,uint width,uint height)
{
