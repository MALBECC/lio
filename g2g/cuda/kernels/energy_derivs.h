                                           ...
                                           uint points, uint m, uint nuc_count)
{
  ...
  for (uint bi = 0; bi < m; bi += DENSITY_DERIV_BATCH_SIZE2) {
    ...
    for (uint i = 0; i < DENSITY_DERIV_BATCH_SIZE2 && (i + bi) < m; i++) {
      ...
      if (valid_thread) {
        uint nuci = nuc_sh[i];
        density_deriv_a[COALESCED_DIMENSION(points) * nuci + point] -= Fgi * w_a; // TODO: esto accede demasiado a memoria, quizas se pueda acumular en sh_mem
        density_deriv_b[COALESCED_DIMENSION(points) * nuci + point] -= Fgi * w_b; // TODO: esto accede demasiado a memoria, quizas se pueda acumular en sh_mem
      }
    }
  }
}
