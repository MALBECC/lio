//-------------------------------------------BEGIN TERM-TYPE DEPENDENT PART (D-S)-------------------------------------------
        ...
        // BEGIN calculation of individual (single primitive-primitive overlap) force terms
        {
          ...
          for (int l1 = 0; l1 < 3; l1++)
          {
            ...
            {
              bool del12 = l1 == l2;
              scalar_type pre_term = mm_charge * dens[dens_ind];
              pre_term *= (1-del12)*1.0f + del12*gpu_normalization_factor;
              //if (valid_thread) printf("%.4e\n", dens[dens_ind]);
              dens_ind++;

              d_s0  = PmA[l1] * (PmA[l2] * F_mU[0] - PmC[l2] * F_mU[1]); // p_s0 (l2)
              d_s0 -= PmC[l1] * (PmA[l2] * F_mU[1] - PmC[l2] * F_mU[2]); // p_s1 (l2)
              d_s0 += del12 * inv_two_zeta * (F_mU[0] - F_mU[1]);
              d_s1  = PmA[l1] * (PmA[l2] * F_mU[1] - PmC[l2] * F_mU[2]); // p_s1 (l2)
              d_s1 -= PmC[l1] * (PmA[l2] * F_mU[2] - PmC[l2] * F_mU[3]); // p_s2 (l2)
              d_s1 += del12 * inv_two_zeta * (F_mU[1] - F_mU[2]);
              for (int grad_l = 0; grad_l < 3; grad_l++)
              {
                bool del1l = l1 == grad_l; bool del2l = l2 == grad_l;
                C_force_term  = del1l * (PmA[l2] * F_mU[1] - PmC[l2] * F_mU[2]); // p_s1 (l2)
                C_force_term += del2l * (PmA[l1] * F_mU[1] - PmC[l1] * F_mU[2]); // p_s1 (l1)
                C_force_term  = PmC[grad_l] * d_s1 + inv_two_zeta * C_force_term;

                AB_common     = del1l * (PmA[l2] * F_mU[0] - PmC[l2] * F_mU[1]); // p_s0 (l2)
                AB_common    += del2l * (PmA[l1] * F_mU[0] - PmC[l1] * F_mU[1]); // p_s0 (l1)
                ...
                A_force_term -= del1l * (PmA[l2] * F_mU[0] - PmC[l2] * F_mU[1]); // p_s0 (l2)
                A_force_term -= del2l * (PmA[l1] * F_mU[0] - PmC[l1] * F_mU[1]); // p_s0 (l1)
