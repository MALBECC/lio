...
void gpu_set_gamma_arrays()
{
  ...
  cudaMallocArray(&gammaArray,&qmmm_str_tex.channelDesc,880,22);
...
#define qmmm_fock_parameters \
  ...
  gpu_partial_fock.data+fock_offset, dens_values.size()/*COALESCED_DIMENSION(num_dens_terms)*/,clatom_pos_gpu.data,clatom_chg_gpu.data//,fock_out_offset
    ...
    for (i = 0; i < NUM_TERM_TYPES; i++)
    {
      ...
      gpu_fock_reduce<scalar_type><<<reduceGridSize,reduceBlockSize,0,stream[i]>>>( gpu_partial_fock.data+fock_offset, dev_dens_values.data+fock_offset, gpu_qmmm_partial_energies.data+energies_offsets[i],
...
template <class scalar_type,bool forces> void g2g_coulomb(double* qm_forces, double& Es, double* Ginv)
{
  ...
  CudaMatrix<double> gpu_partial_fock;
  //
  // Forces: output is partial QM and MM forces
  //
  if (forces)
  ...
  //
    ...
    // Zero the partial Fock matrix on the GPU
    //
    zero_fock<double><<<gridSize,blockSize>>>(gpu_partial_fock.data,dens_values.size(),max_partial_size);
  }
  ...
  uint energies_size = 0;
  CudaMatrix<double> gpu_coulomb_partial_energies;
  if (!forces) {
    for (i = 0; i < NUM_COULOMB_TERM_TYPES; i++) {
      energies_offsets[i] = energies_size;
      energies_size += divUp(dens_counts[i],QMMM_REDUCE_BLOCK_SIZE);
    }
    gpu_coulomb_partial_energies.resize(energies_size,1);
  }
...
#define coulomb_fock_parameters \
  ...
  s_end, p_end, d_end, p_offset, d_offset //,fock_out_offset
    // Each term type is calculated asynchronously
    cudaStream_t stream[NUM_COULOMB_TERM_TYPES];
    for (i = 0; i < NUM_COULOMB_TERM_TYPES; i++) {
      cudaStreamCreate(&stream[i]);
    ...
    for (i = 0; i < NUM_COULOMB_TERM_TYPES; i++)
    {
      ...
      dim3 reduceBlockSize(QMMM_REDUCE_BLOCK_SIZE);
      dim3 reduceGridSize = divUp(reduceThreads,reduceBlockSize);
      gpu_coulomb_fock_reduce<scalar_type><<<reduceGridSize,reduceBlockSize,0,stream[i]>>>( gpu_partial_fock.data+fock_offset, dev_dens_values.data+fock_offset,
                                                                                            gpu_coulomb_partial_energies.data+energies_offsets[i],
                                                                                            dens_values.size(), max_partial_size, dens_counts[i] );
    }
    cudaDeviceSynchronize();
    for (i = 0; i < NUM_COULOMB_TERM_TYPES; i++) {
  ...
  if (forces) {
    ...
    //
    down.start();
    HostMatrix<double> cpu_fock(dens_values.size());
    cudaMemcpy(cpu_fock.data,gpu_partial_fock.data,cpu_fock.bytes(),cudaMemcpyDeviceToHost);
    HostMatrix<double> cpu_partial_energies(gpu_coulomb_partial_energies);
    down.pause_and_sync();

    //
    // Send Fock elements back to RMM(M5) and do final reduction energies into Es
    //
    reduce.start();
    for (uint t = 0; t < NUM_COULOMB_TERM_TYPES; t++) {
      for (i = dens_offsets[t]; i < dens_offsets[t] + dens_counts[t]; i++) {
        uint dens_ind = local2globaldens[i];
        fortran_vars.rmm_output(dens_ind) += cpu_fock(i);
      }
    }
    for (i = 0; i < energies_size; i++) {
      Es += cpu_partial_energies(i);
