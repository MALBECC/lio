    ...
    if (valid_thread) {
      for (int j = 0; j < DENSITY_BLOCK_SIZE; j++) {
        ...
        if ((bj + j) <= i) {
          ...
          // GS density
          rdm_this_thread = fetch(rmm_gpu_3rd_tex, (float)(bj + j), (float)i);
          w  += rdm_this_thread * fjreg;
          w3 += fgjreg * rdm_this_thread;

          // Transition density
          trd_this_thread = fetch(tred_gpu_3rd_tex, (float)(bj + j), (float)i);
        ...
        if (valid_thread2 && ((bj + j) <= i2)) {
          ...
          // GS density
          rdm_this_thread2 = fetch(rmm_gpu_3rd_tex, (float)(bj + j), (float)i2);
          w2  += rdm_this_thread2 * fjreg;
          w32 += fgjreg * rdm_this_thread2;

          // Transition density
          trd_this_thread2 = fetch(tred_gpu_3rd_tex, (float)(bj + j), (float)i2);
