...
texture<int2, 2, cudaReadModeElementType> rmm_gpu_3rd_tex;
#else
texture<float, 2, cudaReadModeElementType> tred_gpu_3rd_tex;
texture<float, 2, cudaReadModeElementType> rmm_gpu_3rd_tex;
#endif
...
// Form Bind Textures
   ...
   // When we did not save the GS density and derivatives
      ...
      cudaMallocArray(&cuArrayrmm, &rmm_gpu_3rd_tex.channelDesc, rmm_cpu.width, rmm_cpu.height);
      cudaMemcpyToArray(cuArrayrmm,0,0,rmm_cpu.data,sizeof(scalar_type)*rmm_cpu.width*rmm_cpu.height,cudaMemcpyHostToDevice);
      cudaBindTextureToArray(rmm_gpu_3rd_tex, cuArrayrmm);
      rmm_cpu.deallocate();
      // CALCULATE PARTIAL DENSITIES
      #define compden_parameter \
         this->number_of_points,function_values_transposed.data,group_m,gradient_values_transposed.data,\
      ...
      #undef compute_parameter
      #undef accumden_parameter
      cudaUnbindTexture(rmm_gpu_3rd_tex);
