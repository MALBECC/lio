   ...
   switch(iexch) {
      ...
      case 2: {
         ...
         if (x1 > 1.0) {
            ...
            vc = (scalar_type)0.0111 * x1 * ((scalar_type)3.0 * t3 * t2 - t1 / (x1 * (x1 + (scalar_type)1.0))
      ...
      case 3: {
         ...
         ec = ( (scalar_type)2.0 * logf(x1) - t2 + POT_VOSKO_2B1Q * t3 
            -   POT_T4 * ((scalar_type)2.0 * log(x1 - POT_VOSKO_X0) 
            -   t2 + POT_VOSKO_B2X0Q * t3) )
            * POT_VOSKO_A1;

         scalar_type vc;
         vc = ec - POT_VOSKO_A16 * x1 * 
            ( t5 / Xx - POT_VOSKO_4B1 / (t1 * t1 + POT_VOSKO_QSQ) * POT_VOSKO_B1X0 
            - POT_T4 * ((scalar_type)2.0 / (x1 - POT_VOSKO_X0) - t1 / Xx));
         y2a = v0 + vc;
      } break;
      default: throw std::runtime_error("Error, invalid iexch"); break;
   }
}
...
{
   switch(iexch) {
      case 0: return calc_ldaCS<scalar_type,0>(dens, ex, ec, y2a);
      case 1: return calc_ldaCS<scalar_type,1>(dens, ex, ec, y2a);
      case 2: return calc_ldaCS<scalar_type,2>(dens, ex, ec, y2a);
      case 3: return calc_ldaCS<scalar_type,3>(dens, ex, ec, y2a);
      case 4: return calc_ldaCS<scalar_type,4>(dens, ex, ec, y2a);
      case 5: return calc_ldaCS<scalar_type,5>(dens, ex, ec, y2a);
      case 6: return calc_ldaCS<scalar_type,6>(dens, ex, ec, y2a);
      case 7: return calc_ldaCS<scalar_type,7>(dens, ex, ec, y2a);
      case 8: return calc_ldaCS<scalar_type,8>(dens, ex, ec, y2a);
      case 9: return calc_ldaCS<scalar_type,9>(dens, ex, ec, y2a);
