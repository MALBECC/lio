...
#define EASYPBE_GAMMA    ((scalar_type)0.03109069086965489503494086371273f) // THIS
#define EASYPBE_BETA     ((scalar_type)0.06672455060314922f) // THIS
#define EASYPBE_DELTA    ((scalar_type)(EASYPBE_BETA/EASYPBE_GAMMA)) // beta/gamma // THIS
...
//  scalar_type ALFC = -ALFM;
   ...
   scalar_type GAM  = pow( (scalar_type) (2.0f), (scalar_type) 4.0f / 3.0f )
                    - (scalar_type)2.0f;
...
// COMM = EC -RS*ECRS/3.D0-ZET*ECZET
   scalar_type ECZET = (scalar_type)4.0f*pow((scalar_type)zet,3)*F*(EP-EU+ALFM/FZZ)
...
// H = G3*(BET/DELT)*DLOG(1.D0+DELT*Q4*T2/Q5)
   ...
   h = G3*(scalar_type)(EASYPBE_BETA/EASYPBE_DELTA)*log((scalar_type)1.0f
     + EASYPBE_DELTA*Q4*T2/Q5);
