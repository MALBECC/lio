...
c
      ...
      dimension c(ng,nl),a(ng,nl),Nuc(M),ncont(M),Iz(nt),q(ntq)
...
c
      write(*,*) ' SCF OPEN SHELL CALCULATION  '
c
c
      good=1.00D0
      niter=0
...
c
        do 10 j=1,M
          if (RMM(M13+j-1).lt.1.0D-07) then
...
c
        ...
        enddo
c
         write(*,*) 'UNPAIRED SPIN MULLIKEN POPULATION ANALYSIS'
         write(*,770)
