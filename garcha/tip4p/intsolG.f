...
c
       if (k1.eq.1) then
        rr(1)=alpha*r(n,1)+alpha2*(r(n+1,1)+r(n+2,1))
        rr(2)=alpha*r(n,2)+alpha2*(r(n+1,2)+r(n+2,2))
        rr(3)=alpha*r(n,3)+alpha2*(r(n+1,3)+r(n+2,3))
       else
        rr(1)=r(n,1)
        rr(2)=r(n,2)
        rr(3)=r(n,3)
...
c
       piNs=t1*s0s(n)-(Q(l2)-rr(l2))*s1s(n)
       sNpi=piNs+tx*s0s(n)
