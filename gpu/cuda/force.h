...
__global__ void calc_forces(const float3* atom_positions, const uint* types, const uint atoms_n, uint nco, uint3 num_funcs,
													 const uint* nuc, const uint* contractions, bool normalize, const float2* factor_ac,
													 const float* rmm, const float* factors, const float3* dds, float3* forces)
{
