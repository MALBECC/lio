...
c
c
       call int2G(NORM,natom,r,Nucd,M,Md,ncontd,nshelld,cd,ad,
     >           af,f)
c     >   ,FXH2,FYH2,FZH2)
...
c
        NCOa=NCO
        NCOb=NCO+Nunp
        call exchnum2op(NORM,natom,r,Iz,Nuc,M,ncont,nshell,c,a,RMM,
     >              M18,NCOa,NCOb,Exc,f)
c     >      ,FXH2,FYH2,FZH2)
       endif
c
...
c-----Control de fzas en int3G
      ...
      fyy=fyy+f(i,2)
      fzz=fzz+f(i,3)
      write(*,*)'INT3G fx fy fz',i,f(i,1),f(i,2),f(i,3)
      enddo

      write(*,*)'INT3G FTOT',dsqrt(fxx**2+fyy**2+fzz**2)

      if(dsqrt(fxx**2+fyy**2+fzz**2).gt.1.D-07)then
      write(*,*)'FZA TOTAL NE ZERO EN  INT3G'
      endif
990   CONTINUE
c--------------------------------------------------------
