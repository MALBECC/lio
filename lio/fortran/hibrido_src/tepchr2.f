      SUBROUTINE TEPCHR2(NATSOL)
      use hibrido_common
        IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION TI(3,3),OMG(3),INDX(3)
     
c -- SOLO REASIGNA VELOCIDADES A LOS ATOMOS DE SOLVENTE
c    (PARA CUANDO NDFT.NE.1 OR NDFT.EQ.1 y NPAS.GT.1)

*-----------Velocid. al azar de 1 distr. gausseana--------------*
      TEMPAV = ZERO
      NOFSET=NSPECQ
      DO  1 I= 1+NSPECQ,NSPECQ+NATSOL
...
*---------Calculo el momento angular total y se lo resto-------*
       ...
       OMG(1) = OMG(1) + WWM(I)*(Y(J)*VZ(J)-Z(J)*VY(J))
       OMG(2) = OMG(2) + WWM(I)*(Z(J)*VX(J)-X(J)*VZ(J))
       OMG(3) = OMG(3) + WWM(I)*(X(J)*VY(J)-Y(J)*VX(J))
8      CONTINUE
       ...
       DO  J = 1+NSPECQ,NPART
       VX(J) = VX(J) - (OMG(2)*Z(J) - OMG(3)*Y(J))
       VY(J) = VY(J) - (OMG(3)*X(J) - OMG(1)*Z(J))
       VZ(J) = VZ(J) - (OMG(1)*Y(J) - OMG(2)*X(J))
       ENDDO
...
*-------------A todo el solvente le  resto la veloc.CM
      DO 9 I=1+NSPECQ,NPART
      VX0(I)=VX(I)-VXCM
      VY0(I)=VY(I)-VYCM
      VZ0(I)=VZ(I)-VZCM
9     CONTINUE
      ...
      TEMPAV=TEMPAV+WWM(I)*(VX0(J)**2+VY0(J)**2
     & +VZ0(J)**2)
45    CONTINUE
      TEMPAV2=TEMPAV/GDF
...
*        TEMPAV2: despues de restar Vcm y Veloc angular  *
...
19     FORMAT (/,2X, 'TERMOSTATO:, TEMP1, TEMP2',G14.6,2X,
     & G14.6,2X,G14.6)
      WRITE (6,*)itel, ' ENTRE A TEPCHR2 y TEMP ES:', TEMPP
