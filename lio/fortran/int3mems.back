...
c-----------------------------------------------------------------
      subroutine int3mems(NORM,natom,r,Nuc,M,Mmem,ncont,nshell,c,a,
...
c
      ...
      INCLUDE 'param'
      parameter(pi52=34.9868366552E0,pi=3.14159265358E0)
...
c
      dimension sipr(nt,3)
      dimension sipcd(ngd,nl),sipad(ngd,nl)
      dimension sipc(ng,nl),sipa(ng,nl)

      dimension Q(3),W(3),HW(3),Rc(ngd),af(ngd),FF(ngd),P(ngd)
...
c     common /index/ iii(ng),iid(ng)
      common /Nc/ Ndens
       logical fato
      real*8 r,cd,ad,c,a,Rc,af,FF,P,RMM,B
      real*4 FUNCTS
c
c------------------------------------------------------------------
...
3666    continue
        ...
        allocate(cools(kknums*Md),kkinds(kknums))
          write(*,*) kknums,kknums*Md,rmax,rmaxs
