...
#  ifdef BPASS_CUDA
      character(len=40), parameter :: BDATA_CUDA      = BPASS_CUDA
#  else
      character(len=40), parameter :: BDATA_CUDA      = 'NO DATA'
#     define INCOMPLETE_BDATA
#  endif
...
#  ifdef BPASS_PARALLEL
      character(len=40),           parameter :: BDATA_PARALLEL  = BPASS_PARALLEL
#  else
      character(len=40),           parameter :: BDATA_PARALLEL  = 'NO DATA'
#     define INCOMPLETE_BDATA
#  endif


#  ifdef BPASS_PRECISION
      character(len=40),           parameter :: BDATA_PRECISION = BPASS_PRECISION
#  else
      character(len=40),           parameter :: BDATA_PRECISION = 'NO DATA'
#     define INCOMPLETE_BDATA
#  endif
...
contains
   ...
   subroutine print_build_info( outunit )
      ...
      write( unit=outunit, fmt= '(2A)') '  *  cuda      = ', BDATA_CUDA
      write( unit=outunit, fmt= '(A,I2)') '  *  intel     = ', BDATA_INTEL
      write( unit=outunit, fmt= '(2A)') '  *  parallel  = ', BDATA_PARALLEL
      write( unit=outunit, fmt= '(2A)') '  *  precision = ', BDATA_PRECISION
