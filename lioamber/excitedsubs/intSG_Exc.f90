...
subroutine intSG_Exc(ff, Xmat, natom, M)
   ...
   do jfunct = 1, ifunct
      ...
      do ncj = 1, ncont(jfunct)
         ...
         te = Xmat(ifunct,jfunct) * ccoef * 2.0D0 * ss
         t4 = te * a(ifunct,nci)
         t5 = te * a(jfunct,ncj)

         te_t = Xmat(jfunct,ifunct) * ccoef * 2.0D0 * ss
         t4_t = te_t * a(ifunct,nci)
         t5_t = te_t * a(jfunct,ncj)
         ...
         do l1 = 1, 3
            ...
            ! prueba
            ff(Nuc(ifunct),l1) = ff(Nuc(ifunct),l1) - t4_t * t1
            ff(Nuc(jfunct),l1) = ff(Nuc(jfunct),l1) + t5_t * (t1 + &
                                  (r(Nuc(ifunct),l1) - r(Nuc(jfunct),l1)))
         enddo
      enddo
      enddo
  ...
  do jfunct = 1   , ns
     ...
     do ncj = 1, ncont(jfunct)
        ...
        do l1 = 1, 3
           ...
           te = Xmat(ifunct+l1-1,jfunct) * ccoef  * ss
           t4 = 2.D0 * te * a(ifunct,nci)
           t5 = 2.D0 * te * a(jfunct,ncj)

           te_t = Xmat(jfunct,ifunct+l1-1) * ccoef  * ss
           t4_t = 2.D0 * te_t * a(ifunct,nci)
           t5_t = 2.D0 * te_t * a(jfunct,ncj)
           ...
           do l2 = 1, 3
              ...
              if (l1 .eq. l2) then
                 ...
                 ff(Nuc(ifunct),l2) = ff(Nuc(ifunct),l2) - te_t! mio
                 ds = ds + 1.0d0 / Z2
                 pp = pp + 1.0d0 / Z2
              endif
              ...
              ! prueba
              ff(Nuc(ifunct),l2) = ff(Nuc(ifunct),l2) - t4_t * ds
              ff(Nuc(jfunct),l2) = ff(Nuc(jfunct),l2) + t5_t * pp
           enddo
        enddo
     enddo
     enddo
      ...
      do ncj = 1, ncont(jfunct)
         ...
         do l1 = 1, 3
            ...
            do l2 = 1, lij
               ...
               te = Xmat(ifunct+l1-1,jfunct+l2-1) * ccoef
               t5 = te * 2.D0 * a(jfunct,ncj)
               t4 = te * 2.D0 * a(ifunct,nci)

               te_t = Xmat(jfunct+l2-1,ifunct+l1-1) * ccoef
               t5_t = te_t * 2.D0 * a(jfunct,ncj)
               t4_t = te_t * 2.D0 * a(ifunct,nci)
               ...
               do l3 = 1, 3
                  ...
                  if (l1 .eq. l3) then
                     ...
                     ff(Nuc(ifunct),l3) = ff(Nuc(ifunct),l3) - te_t * spj! mio
                  endif
                  if (l2 .eq. l3) then
                     dp=dp+t11
                     ff(Nuc(jfunct),l3) = ff(Nuc(jfunct),l3) - te * pis 
                     ff(Nuc(jfunct),l3) = ff(Nuc(jfunct),l3) - te_t * pis! mio
                  endif
                  ...
                  ! prueba
                  ff(Nuc(ifunct),l3) = ff(Nuc(ifunct),l3) - t4_t * dp
                  ff(Nuc(jfunct),l3) = ff(Nuc(jfunct),l3) + t5_t * pd
               enddo
            enddo
         enddo
      enddo
