...
subroutine int3G(f,calc_energy)
   ...
   double precision :: f(natom,3)

   double precision :: Q(3), W(3), Jx(ng), af2(ngd), ftot(3)
   double precision :: Exc, alf, cc, ccoef, rexp, ro, roz, sq3
   double precision :: term, u
   ...
   double precision :: dp1s, dp2p, dpc, dpd, dpf, dpk, dpp, dps, ds
   ...
   double precision :: te, tee, tw, twe, tx, ty, tye, tz, tze
   ...
   ! calculated again.
   ! V(i,j) obtained by adding af(k) * t(i,j,k)
   !------------------------------------------------------------------
   ...
   ! iforce 0, numerical integration for exchange-correlation terms
   ! iforce 1, approximate analytical expression is used
   do k=1,Md
      B(k,2)=0.0D0
   enddo
