...
c      parameter (rmaxs=150.D0)
      dimension sipr(natom,3)
      dimension sipcd(Md,nl),sipad(Md,nl)
      dimension sipc(M,nl),sipa(M,nl)

      dimension Q(3),W(3),HW(3),Rc(Md),FF(Md),P(Md)
      dimension dss(natom,natom),Jx(M)
      logical fato
*****
*****
c scratch space
...
c
      do 5 i=1,natom
      do 5 j=1,natom
       dss(i,j)=(r(i,1)-r(j,1))**2+(r(i,2)-r(j,2))**2+
     >        (r(i,3)-r(j,3))**2
 5    continue
c
      do 6 k=1,Md
...
c
       ...
       alf=sipa(i,ni)*tj
       rexp=alf*dd
       if (rexp.lt.rmax.and.rexp.gt.rmaxs) then
       if (fato) then
       kknums = kknums+1
        fato=.false.
       endif
...
c          write(*,*) kknums,kknums*Md,rmax,rmaxs
...
c
      do 11 nk=1,ncontd(k)
c
      ccoef=sipc(i,ni)*sipc(j,nj)*sipcd(k,nk)
      t0=sipad(k,nk)+zij
      u=sipad(k,nk)*zij/t0*dpc
      t1=sipad(k,nk)*sqrt(t0)
c      funct=FUNCTS(0,u)
      term=sks/t1*FUNCTS(0,u)
      term=term*ccoef
...
c
      f2=1.D0
      if (l4.eq.l5) then
       term=term+(dp-ro*d1p/sipad(k,nk))/zc
       f2=sq3
      endif
c
      l12=Ll(l1)+l2
