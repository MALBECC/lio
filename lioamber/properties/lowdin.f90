...
! and printing.
subroutine print_lowdin_cs(Pmat, Smat, atom_of_func, atom_z)
   ...
   call write_population(atom_z, q, 2, UIDs%mul, flowdin)

   deallocate(q)
   call g2g_timer_sum_pause('Lowdin')
end subroutine print_lowdin_cs

subroutine print_lowdin_os(Pmat, Pmat_b, Smat, atom_of_func, atom_z)
   use properties_data, only: UIDs, flowdin
   ...
   call write_population(atom_z, q, 2, UIDs%low, flowdin)

   spinfile = trim(flowdin) // "_spin"
   call write_population(atom_z, s, 3, UIDs%lows, spinfile)
